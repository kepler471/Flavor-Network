...
def split_ingr(x):
    ...
    cleanlist = []
    lst = x.strip('[]').split(',')
    cleanlist = [' '.join(wnl.lemmatize(word.lower()) for word in word_tokenize(re.sub('[^a-zA-Z]', ' ', item))) for
                 item in lst]
    return cleanlist
...
def remove_word(word):
    alist = ['low fat', 'reduced fat', 'fat free', 'fatfree', 'nonfat', 'gluten free', 'free range', \
             'reduced sodium', 'salt free', 'sodium free', 'low sodium', 'sweetened', 'unsweetened', 'large',
             'extra large', 'oz ']
    for item in alist:
...
def cleanup_ingredients(ingr, df, col):
    ...
    long_ingredients = [x for x in ingr if ' ' in x]
    ...
    print(len(df_ingr), len(list(df_dic.keys())), len(diff_dic))
...
def tweak_dic(df_dic, diff_df):
    ...
    for pepper in alist:
        for item in [x for x in diff_df if pepper in x]:
            if item not in df_dic:
                df_dic[item] = ['tabasco pepper']

    for item in [x for x in diff_df if 'flour' in x]:
        if item not in df_dic:
            df_dic[item] = ['whole grain wheat flour']

    for item in [x for x in diff_df if 'tumeric' in x]:
        if item not in df_dic:
            df_dic[item] = ['turmeric']

    for item in [x for x in diff_df if 'yoghurt' in x]:
        if item not in df_dic:
            df_dic[item] = ['yogurt']

    for item in [x for x in diff_df if 'sausage' in x]:
        if item not in df_dic:
            df_dic[item] = ['smoked sausage']

    alist = ['rib', 'chuck', 'sirloin', 'steak']
    for beef in alist:
        for item in [x for x in diff_df if beef in x]:
            if item not in df_dic:
                df_dic[item] = ['beef']

    for item in [x for x in diff_df if 'fillet' in x]:
        if item not in df_dic:
            df_dic[item] = ['raw fish']

    for item in [x for x in diff_df if 'mozzarella' in x]:
        if item not in df_dic:
            df_dic[item] = ['mozzarella cheese']

    for item in [x for x in diff_df if 'spinach' in x]:
        if item not in df_dic:
            df_dic[item] = ['dried spinach']

    for item in [x for x in diff_df if 'curry' in x]:
        if item not in df_dic:
